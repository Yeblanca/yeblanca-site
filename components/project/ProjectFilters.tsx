...
export function ProjectFilters({ projects }: { projects: ProjectCardData[] }) {
  ...
  function FilterBar({
    ...
    return (
      <div className="flex items-center gap-2 flex-wrap">
        <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-[rgba(240,240,240,0.30)] mr-1">
          {label}
        </span>
        <button
  ...
  return (
    <div>
      ...
      {/* Grid */}
      {filtered.length === 0 ? (
        <p className="font-mono text-[11px] uppercase tracking-[0.10em] text-[rgba(240,240,240,0.40)] py-16 text-center">
          {t('no_results')}
        </p>
      ) : (
